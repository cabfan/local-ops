...
- 新增 Linux systemd 用户服务安装脚本 `tools/install-systemd.sh`，一键安装 `local-ops-console.service` 并启用开机自启；服务以后台 `--no-browser` 方式运行，页面重启在 systemd 托管时走 `systemctl --user restart`。
...
- 修复从局域网地址访问总控台时，启动台/服务监控点击端口仍打开本机回环地址的问题；现在会使用当前访问控制台的主机名打开服务端口（本机回环访问仍按实际监听地址处理）。
