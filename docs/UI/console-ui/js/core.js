...
function loadThemeCss(name) {
  ...
  /* 相对路径：页面托管在根路径或任意子路径都能解析 */
  const href = './themes/' + name + '.css';
  const currentUrl = new URL(themeCss.getAttribute('href') || href, location.href).href;
  const targetUrl = new URL(href, location.href).href;
  if (currentUrl === targetUrl && themeCss.sheet) return Promise.resolve();
  return new Promise((resolve, reject) => {
    ...
    themeCss.href = href + (currentUrl === targetUrl ? '?retry=' + Date.now() : '');
  });
}
